...
QString weburl1 = "http://www.google.com";
ui.webView->load(QUrl(weburl));
//ui.webView->load(QUrl("http://www.google.com"));
ui.webView->show();
UT::getInstance()->LogToFile("Authentication DONE"); 
}

void MainWindowContainer::sslErrorHandler(QNetworkReply* qnr, const QList<QSslError> & errlist)
{
...
{   
	...
	if(!HttpClient::getInstance()->finishedNetWorkSlot(reply,networkError) )
	{
		...
		default:
			char resp[100];
			UT::getInstance()->LogToFile("ERROR STRING");  
			UT::getInstance()->LogToFile(reply->errorString());  
			UT::getInstance()->LogToFile("READ ALL");  
			UT::getInstance()->LogToFile(reply->readAll());  
			UT::getInstance()->LogToFile("http error number"); 
			sprintf(resp, "%i", reply->attribute( QNetworkRequest::HttpStatusCodeAttribute).toInt());
			UT::getInstance()->LogToFile(resp);
			UT::getInstance()->LogToFile("qt error number"); 
			sprintf(resp, "%i", networkError);
			UT::getInstance()->LogToFile(resp);
			sprintf(resp, "<html><body>NO NETWORK CONNECTION !! Default error %i %i </body></html>", networkError, reply->attribute( QNetworkRequest::HttpStatusCodeAttribute).toInt() );
			//ui.webView->setHtml(resp);
			break;
		}
	} 
	else 
	{
		...
		{
			ui.webView->setHtml("<html><body> User not authorised</body></html>");	
		}
	}
}
...
void MainWindowContainer::adjustLocation()
{
   ...
   UT::getInstance()->LogToFile("Page loaded:" + ui.webView->url().toString()); 
   /*
...
{	
	progress = 100;
    adjustTitle();
	UT::getInstance()->LogToFile("Authentication page loaded!"); 
}
